...
def showWeather():
    ...
    api_key = "d9bf722777bb1e016002fb8a08a080e1"  # sample API
    ...
    if weather_info['cod'] == 200:
        ...
        weather = f"\nWeather of: {city_name}\nTemperature : {temp}°C\nFeels like in : {feels_like_temp}°C\nPressure: {pressure} hPa\nHumidity: {humidity}%\nSunrise at {sunrise_time} and Sunset at {sunset_time}\nCloud: {cloudy}%\nInfo: {description}"
    else:
...
def get_label(text):
    city_head = Label(root, text=text, font='Arial 12 bold').pack(pady=10)  # to generate label heading
    return city_head
...
# Button.bind("<Return>", showWeather)

# Create a button for submitting the search
# button = tk.Button(root, text="Search", command=showWeather)
